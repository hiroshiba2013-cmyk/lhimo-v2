...
-- Adattato allo schema del nuovo progetto (lrqeojukjpjllnvsjtor)
-- ============================================================

-- Add search_vector column to businesses if missing
...
-- Note: handle_new_user() already exists, this adds a second trigger name
-- ============================================================
...
-- NOTE: column is added_by_family_member (not added_by_family_member_id)
-- ============================================================
...
BEGIN
  ...
  v_family_member_id := NEW.added_by_family_member;
  ...
  INSERT INTO activity_log (
    user_id, family_member_id, activity_type, title, description,
    points_earned, icon, color, metadata, created_at
  ) VALUES (
    NEW.added_by, v_family_member_id, 'business_added',
    'Attivita'' aggiunta',
    CASE 
      WHEN is_complete THEN 'Hai aggiunto "' || NEW.name || '" con dati completi'
      ELSE 'Hai aggiunto "' || NEW.name || '"'
    END,
    points_to_award, 'building', 'green',
    jsonb_build_object('business_id', NEW.id, 'business_name', NEW.name, 'is_complete', is_complete, 'family_member_id', v_family_member_id),
    now()
  );
...
BEGIN
  ...
  AND (NEW.points_awarded IS NOT TRUE) THEN
    ...
    IF NEW.added_by IS NOT NULL THEN
      ...
      INSERT INTO activity_log (
        ...
        NEW.added_by, 'business_added', 'Attivita'' approvata',
        CASE 
      ...
      VALUES (
        NEW.added_by, v_family_member_id, 'business_approved',
        'Attivita'' Approvata',
  ...
  AND NEW.added_by IS NOT NULL THEN
    ...
    INSERT INTO notifications (user_id, family_member_id, type, title, message, data)
    VALUES (
      NEW.added_by, v_family_member_id, 'business_rejected',
      'Attivita'' Rifiutata',
...
BEGIN
  ...
  SET total_points = total_points + points_to_award, updated_at = now()
  WHERE user_id = NEW.added_by;

  INSERT INTO activity_log (
    user_id, activity_type, title, description, points_earned, icon, color, metadata, created_at
  ) VALUES (
    NEW.added_by, 'business_added', 'Attivita'' aggiunta',
    CASE 
      WHEN is_complete THEN 'Hai aggiunto "' || NEW.name || '" con dati completi'
      ELSE 'Hai aggiunto "' || NEW.name || '"'
    END,
    points_to_award, 'building', 'green',
    jsonb_build_object('business_id', NEW.id, 'business_name', NEW.name, 'is_complete', is_complete),
    now()
  );
...
-- NOTE: column is tax_code (not fiscal_code) in customer_family_members
-- ============================================================
...
BEGIN
  UPDATE user_activity
  SET ads_posted_count = GREATEST(0, ads_posted_count - 1),
...
DECLARE
  v_family_member_id uuid;
BEGIN
  v_family_member_id := OLD.added_by_family_member;
...
BEGIN
  IF NEW.added_by_family_member IS NOT NULL THEN
    SELECT EXISTS(
      SELECT 1 FROM user_activity
      WHERE user_id = NEW.added_by AND family_member_id = NEW.added_by_family_member
    ) INTO v_exists;
    ...
    ELSE
      INSERT INTO user_activity (user_id, family_member_id, businesses_added_count, last_activity_at, created_at, updated_at)
      VALUES (NEW.added_by, NEW.added_by_family_member, 1, now(), now(), now());
    END IF;
  ELSE
    SELECT EXISTS(
      SELECT 1 FROM user_activity
  ...
  IF NEW.user_type = 'business' AND NEW.subscription_status = 'trial' THEN
    ...
    IF basic_plan_id IS NOT NULL THEN
      INSERT INTO subscriptions (
        customer_id, plan_id, status, start_date, end_date, trial_end_date, payment_method_added
      ) VALUES (
        NEW.id, basic_plan_id, 'trial', now(), trial_end, trial_end, false
      )
...
BEGIN
  ...
  INSERT INTO notifications (user_id, family_member_id, type, title, message, data)
  VALUES (
    NEW.reporter_id, NEW.family_member_id, 'report_submitted',
    'Segnalazione inviata',
    'La tua segnalazione per questo ' || v_entity_label || ' e'' stata ricevuta e verra'' esaminata dal nostro staff.',
    jsonb_build_object('entity_type', NEW.reported_entity_type, 'entity_id', NEW.reported_entity_id, 'report_id', NEW.id)
  );
...
-- NOTE: customer_family_members uses tax_code, not fiscal_code
-- register_trial_usage() doesn't exist - replaced with inline SQL
-- ============================================================
  ...
  IF NEW.status = 'trial' AND (OLD IS NULL OR OLD.status != 'trial') THEN
    ...
    IF v_profile_fiscal_code IS NOT NULL AND v_profile_fiscal_code != '' THEN
      ...
      IF v_existing_count > 0 THEN
        RAISE EXCEPTION 'Questo codice fiscale e'' gia'' stato utilizzato per un periodo di prova';
      END IF;
    END IF;
    ...
    IF v_family_fiscal_codes IS NOT NULL THEN
      ...
      IF v_existing_count > 0 THEN
        RAISE EXCEPTION 'Uno o piu'' codici fiscali dei membri della famiglia sono gia'' stati utilizzati per un periodo di prova';
      END IF;
    END IF;

    -- Register trial usage inline (register_trial_usage function doesn't exist)
    IF v_profile_fiscal_code IS NOT NULL AND v_profile_fiscal_code != '' THEN
      INSERT INTO trial_usage_history (fiscal_code, first_user_id, first_trial_date)
      VALUES (v_profile_fiscal_code, NEW.customer_id, now())
      ON CONFLICT (fiscal_code) DO NOTHING;
    END IF;

    IF v_family_fiscal_codes IS NOT NULL THEN
      INSERT INTO trial_usage_history (fiscal_code, first_user_id, first_trial_date)
      SELECT unnest(v_family_fiscal_codes), NEW.customer_id, now()
      ON CONFLICT (fiscal_code) DO NOTHING;
    END IF;
  END IF;
...
BEGIN
  ...
  IF OLD.family_member_id IS NOT NULL THEN
    UPDATE user_activity
    SET total_points = GREATEST(0, total_points - points_to_subtract),
...
BEGIN
  UPDATE auctions
  SET current_price = NEW.bid_amount, updated_at = now()
  WHERE id = NEW.auction_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  VALUES (NEW.customer_id, 15, 1, now(), now())
